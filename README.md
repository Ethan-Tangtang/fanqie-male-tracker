...
- **平台评分**：仅在番茄公开页面能读取时展示；无法读取时明确显示“暂无公开评分”。
- **质量指数**：项目按公开排名、在读人数、相对上一份快照的变化计算的 0–100 分指标，用于稳定展示高质量作品和潜力新书；它不是番茄官方评分。
- **追踪天数**：同一本书在本项目连续快照中出现的天数。
- **在读变化**：与上一份项目快照相比的在读数变化。

番茄面向普通访客的公开页面不提供单本书的读者停留时长、完读率或个人浏览历史；这些指标不会在本项目中伪造。项目以“追踪天数”和“在读变化”作为长期表现的公开代理指标。
